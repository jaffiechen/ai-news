...
### ⚡ 性能优化
- **无限滚动加载**：首屏仅渲染 30 条，滚动时按需加载，大幅减少初始 DOM 节点数
  - 支持 7000+ 条新闻流畅滚动
  - requestAnimationFrame 节流，避免卡顿
  - 显示加载进度（当前/总数）

- **搜索倒排索引**：基于分词的倒排索引实现，搜索性能大幅提升
  - 支持多关键词 AND 搜索
  - 索引自动构建与缓存
  - 数据变化时自动重建

- **CSS Content Visibility**：给新闻卡片添加 content-visibility: auto
  - 视口外的卡片跳过渲染
  - 初始渲染速度提升 30-50%
  - 零成本优化

- **日期格式化缓存**：formatDate/formatHour/formatDateTime 结果缓存
  - 避免重复 new Date() 和字符串拼接
  - 相对时间 60 秒内复用缓存

- **PWA 缓存策略分级**：
  - 静态资源（JS/CSS/图片）：CacheFirst（缓存优先，30 天）
  - 新闻数据（JSON）：StaleWhileRevalidate（先显示缓存，后台更新）
  - HTML 文档：NetworkFirst（网络优先）
  - 首屏加载速度显著提升

### 🐛 修复
- **客户端时间过滤问题**：移除前端重复的时间范围过滤，解决站点数量与实际列表不一致问题
  - 新智元、YouTube、微信公众号等站点点击后"暂无新闻"的问题
