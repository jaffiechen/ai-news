...
const { getFavorites, resetFavorites, toggleFavorite } = useStorage()
...
function handleRemove(item: Favorite, e: Event) {
  e.preventDefault()
  e.stopPropagation()
  toggleFavorite({ id: item.newsId, title: item.title, url: item.link, source: item.source } as any)
  favoritesList.value = getFavorites()
}
        ...
        <div class="w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden max-h-[80vh] flex flex-col">
          <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white">我的收藏</h3>
              <span class="text-sm text-gray-500">({{ favoritesList.length }})</span>
            </div>
            <button 
              @click="close"
          ...
          <div class="flex-1 overflow-y-auto">
            <div v-if="favoritesList.length === 0" class="flex flex-col items-center justify-center py-16 text-gray-400">
              <div class="text-5xl mb-3">⭐</div>
              <p class="text-sm">暂无收藏</p>
              <p class="text-xs mt-1">点击新闻卡片上的星标即可收藏</p>
            </div>
      ...
      >
        <div class="w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden p-6">
          <div class="text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">确认清空收藏？</h3>
            <p class="text-sm text-gray-500 mb-6">
              将删除 {{ favoritesList.length }} 条收藏<br>
              此操作无法恢复
            </p>
          </div>
